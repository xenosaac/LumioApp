...
class SleepManager: ObservableObject {
    ...
    func fetchSleepData() async {
        guard HKHealthStore.isHealthDataAvailable(), !isUsingMockData else { return }
...
extension SleepManager {
    /// 计算单晚睡眠分数（0-100, 支持小数）
    /// - Parameter nightData: 某一晚的 SleepDataPoint 数组
    /// - Returns: 0-100 分（小数）
    static func calculateSleepScore(for nightData: [SleepDataPoint]) -> Double {
        let sleepStages: [SleepStage] = [.asleep, .deep, .rem, .core]
        let totalMinutes = nightData.filter { sleepStages.contains($0.sleepStage) }
            .reduce(0.0) { $0 + $1.durationInMinutes }
        let deepMinutes = nightData.filter { $0.sleepStage == .deep }
            .reduce(0.0) { $0 + $1.durationInMinutes }
        let remMinutes = nightData.filter { $0.sleepStage == .rem }
            .reduce(0.0) { $0 + $1.durationInMinutes }
        let awakeCount = nightData.filter { $0.sleepStage == .awake }.count
        ...
        let durationScore: Double = {
            switch hours {
            case 7...9: return 60
            case 6..<7, 9..<10: return 50
            case 5..<6, 10..<11: return 40
            case 4..<5, 11..<12: return 30
            default: return 20
            }
        }()
        // 深度睡眠比例（25分）
        let deepScore: Double = {
            switch deepRatio {
            case 0.15...0.25: return 25
            case 0.10..<0.15, 0.25..<0.30: return 18
            case 0.05..<0.10, 0.30..<0.35: return 10
            default: return 0
            }
        }()
        // REM睡眠比例（10分）
        let remScore: Double = {
            switch remRatio {
            case 0.18...0.22: return 10
            case 0.15..<0.18, 0.22..<0.25: return 7
            case 0.10..<0.15, 0.25..<0.30: return 4
            default: return 0
            }
        }()
        // 觉醒次数（5分）
        ...
        return min(durationScore + deepScore + remScore + awakeScore, 100)
    }
} 
